...
const ServiceItem = ({ service, isMobile, setClickedСategory, subServicesRef }) => {
    ...
    return (
        <div className="services-item d-flex text-center mx-auto">
            ...
            <div className={`services-text d-flex flex-column justify-content-center align-items-center text-center ${isEven ? 'text-left' : 'text-right'}`}>
                <div className='service-text-container'>
                    <h3>{service.category}</h3>
                    <p>{service.description}</p>
                </div>
                <button onClick={handleClick} className="view-more-btn border-0 cursor-pointe">
                    View more
                </button>
            </div>
            {!isMobile && isEven && servImg}
