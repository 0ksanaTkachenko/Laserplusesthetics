...
const ServicesItem = ({ subService }) => {
    ...
    return (
        <>
            <div className='sub-services d-flex flex-column' onClick={handleShow}>
                <div className='sub-services-name'>{subService.name}</div>
                <div className='sub-services-img'>
                    <img src={subService.img} className='img-fluid w-100 h-100' alt={subService.name} />
                </div>
            </div>
            <ServicesModal show={showModal} handleClose={handleClose} subService={subService} />
        </>
    );
};
...
const ServiceFilter = ({ clickedСategory, subServicesRef }) => {
    ...
    const categories = useMemo(() => ["All", ...services.map(service => service.category)], []);
    ...
    const displayedServices = useMemo(() => {
        const flatServices = filteredServices.flatMap(service => service.services);
    ...
    const hasMoreServices = useMemo(() => (
        filteredServices.flatMap(service => service.services).length > (isMobile ? 2 : 3)
    ), [filteredServices, isMobile]);

    return (
        <div className="service-filter text-center">
            ...
            <div className="services-list">
                <div className="sub-services-container" ref={subServicesRef}>
            ...
            {hasMoreServices && (
                <div className="show-more-container w-100">
                    <button
                        onClick={() => setShowAll(prev => !prev)} className="show-more-btn cursor-pointer border-0 bg-transparent w-100 mx-auto justify-content-center d-flex align-items-center">
                    {showAll ? (
                        ...
                        <span className="btn-text">{showAll ? "Show Less" : "Show More"}</span>
                    </button>
                </div>
            )}
        </div>
